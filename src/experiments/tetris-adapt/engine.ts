...
export type TrialKind = 'smr' | 'overlap'
...
export const PLANS: Record<PlanId, PlanSpec> = {
  quick: {
    id: 'quick',
    label: '快速',
    hint: '左移/右移 20 + 旋转/下落 20，再宽 5 / 宽 7 各 20，约 80 试次',
    blocks: [
  ...
  standard: {
    id: 'standard',
    label: '标准',
    hint: '左移/右移 40 + 旋转/下落 40，再宽 5 / 宽 7 各 40，约 160 试次',
    blocks: [
...
export function instructionFor(trial: TrialRecord | null): string {
  if (!trial) return '选择计划后开始。先在完整井里左移、右移、旋转、下落，再在窄井上对齐落点。'
...
export function overallHitRate(trials: readonly TrialRecord[]): number | null {
  const done = trials.filter((trial) => trial.outcome)
  if (done.length === 0) return null
