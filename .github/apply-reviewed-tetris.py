"""Apply the reviewed, hash-locked text edits; never execute manifest content."""
import base64
import gzip
import hashlib
import json
...
EXPECTED = 'cdce1c3b3dd104824899de88ac033217ee7f2bab648addec694b06d58e0db2d1'
...
for item in plan:
    ...
    if not (name.startswith(('src/', 'scripts/', 'tests/')) or name.startswith('vite.') and '/' not in name):
        raise SystemExit('Edit is outside the reviewed source scope: ' + name)
    seen.add(name)
    file = root / name
...
print(f'Applied {len(prepared)} reviewed source files with before/after SHA-256 verification.')
