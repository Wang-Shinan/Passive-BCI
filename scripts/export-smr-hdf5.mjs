...
function main() {
  const ncc = findNcc()
  if (!ncc) {
    console.error('[export-smr] 找不到 NCC-OI-BCI。请设置 NCC_OI_BCI_ROOT。')
    process.exit(1)
  }
  const python = findPython(ncc)
  const recordings = path.join(passiveRoot, 'recordings')
  const args = ['scripts/export_smr_control_hdf5.py', '--recordings', recordings, ...extraArgs()]
  console.log(`[export-smr] NCC=${ncc}`)
  console.log(`[export-smr] python=${python}`)
  ...
  child.on('error', (error) => {
    console.error(`[export-smr] 无法启动 Python: ${error.message}`)
    process.exit(1)
  })
