/**
 * Follow another machine's REVE online-learn TCP JSONL stream.
 *
 *   npm run reve:follow -- --host 192.168.1.8
 */
...
function main() {
  const ncc = findNcc()
  if (!ncc) {
    console.error('[follow] 找不到 NCC-OI-BCI。请设置 NCC_OI_BCI_ROOT。')
    process.exit(1)
  }
  const python = findPython(ncc)
  const args = ['scripts/follow_reve_tcp.py', ...extraArgs()]
  console.log(`[follow] NCC=${ncc}`)
  console.log(`[follow] python=${python}`)
  ...
  child.on('error', (error) => {
    console.error(`[follow] 无法启动 Python: ${error.message}`)
    process.exit(1)
  })
